        ...
        if not Path(ckpt_exportdir.local_filename).exists():
            ...
            if not Path(ckpt_exportdir.local_filename).exists() and not Path(ckpt_exportdir.local_filename).is_symlink():
                try:
                    ...
                    # Windows without admin privileges: use directory junction or skip
                    import subprocess
                    try:
                        subprocess.run(['cmd', '/c', 'mklink', '/J', str(ckpt_exportdir.local_filename), str(cache_dir.local_filename)], 
                                      check=True, capture_output=True)
                    except Exception:
                        import shutil
                        if Path(cache_dir.local_filename).exists():
                            shutil.copytree(cache_dir.local_filename, ckpt_exportdir.local_filename, dirs_exist_ok=True)
