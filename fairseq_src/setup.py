...
# Skip C extensions on Windows if no compiler is available
...
if sys.platform == "win32":
    try:
        ...
        print("WARNING: MSVC compiler not found, skipping C/C++ extensions. "
              "This is fine for wav2vec2 usage.")
...
if __name__ == "__main__":
    try:
        ...
        _copied_examples = False
        if "build_ext" not in sys.argv[1:] and not os.path.exists(fairseq_examples):
            try:
                os.symlink(os.path.join("..", "examples"), fairseq_examples)
            except OSError:
                # Windows without admin privileges: copy instead of symlink
                import shutil
                shutil.copytree(os.path.join(".", "examples"), fairseq_examples)
                _copied_examples = True
        ...
        if "build_ext" not in sys.argv[1:]:
            ...
            elif _copied_examples and os.path.isdir(fairseq_examples):
                import shutil
